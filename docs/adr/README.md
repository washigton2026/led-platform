...
| [0017](0017-blackout-intencional-vs-heartbeat.md) | Blackout intencional × invariante do heartbeat: a máscara vive no `OutputManager`, **a jusante** do `record()` — o preto persiste sem ser gravado | aceito (máscara impl.; D6 aberto) | 2026-07-26 |
...
| [0030](0030-portas-fisicas-subdivisao-de-enderecamento.md) | Portas físicas: a porta é subdivisão de **endereçamento**, e a repartição tem um só dono | **implementado** (FASE C, 2026-09-01) — pendência do §5 (`max_pixels % ports != 0`) por decidir, com o gatilho **por disparar** | 2026-08-30 |
| [0031](0031-negociacao-de-versao-no-handshake.md) | Negociação de versão no handshake: o `hello` é o chão, e a ligação tem um dialecto | aceito (pré-impl.) | 2026-08-31 |
