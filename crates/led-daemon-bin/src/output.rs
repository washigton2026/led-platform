...
use led_core::{CompiledLayout, LogicalFrame, OutputError, ProtocolOutput, RgbOrder};
use led_hal::Hal;
...
impl OutputConfig {
    ...
    pub fn parse(s: &str, pixel_count: usize, first_universe: u16) -> Result<Self, String> {
        ...
        Ok(Self { protocol, addr, pixel_count, first_universe })
    }
}
...
impl OutputManager {
    ...
    pub fn open(cfg: OutputConfig) -> std::io::Result<Self> {
        let out: Box<dyn ProtocolOutput> = match cfg.protocol {
            OutputProtocol::Ddp => {
                Box::new(led_player::DdpOutput::new(cfg.addr, cfg.pixel_count)?)
            }
            OutputProtocol::ArtNet => {
                let assigns =
                    linear_assignments(cfg.pixel_count, 1, cfg.first_universe, RgbOrder::Rgb);
