...
**TD-015 aberto por caminho.** Ao verificar o fechamento documental do F-01 encontrei que `main.rs` — a superfície da CLI — é o **único** dos dez ficheiros de `led-console-bin/src/` fora das `FONTES` do `surface_gate`, e portanto escapa aos três gates textuais do crate. A causa não é esquecimento simples: `main.rs:319` tem, dentro do próprio `mod tests`, a lista `["blackout", "--auth", "--cors", "0.0.0.0"]` que **impõe** a mesma regra ao `--help` — e `linhas_de_codigo` só filtra comentários, não `#[cfg(test)]`. Acrescentá-lo faria o gate reprovar por causa de um teste que existe para o mesmo fim. E os outros **nove têm todos `mod tests` inline**: passam por sorte, não por desenho. É a mesma classe que a F1-B corrigiu e não fechou — *"um gate não pode ser o sítio onde o proibido é escrito"*. **Decisão, não edição**, porque ensinar o gate a parar em `#[cfg(test)]` muda o que os nove passam a ser verificados contra.
